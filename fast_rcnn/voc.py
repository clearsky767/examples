...
class TransformVOCDetectionAnnotation(object):
    ...
    def __call__(self, target):
        ...
        res = {
            'boxes': torch.DoubleTensor(boxes),
...
class VOCDetection(data.Dataset):
    ...
    def show(self, index):
        img, t = self.__getitem__(index)
        draw = ImageDraw.Draw(img)
        target = zip(t["gt_classes"],t["boxes"])
        for idx,box in target:
            box = box.numpy().tolist()
            draw.rectangle(box, outline=(255,0,0))
            draw.text(box[0:2], "class idx is {}".format(idx), fill=(0,255,0))
        img.show()
...
if __name__ == '__main__':
    ...
    ds = VOCDetection('/media/alex/my_disk/VOCdevkit/', 'train',
