...
open = cv2.morphologyEx(imgSkin, cv2.MORPH_OPEN, element)
close = cv2.morphologyEx(imgSkin, cv2.MORPH_CLOSE, element)

# display original image and skin image
plt.subplot(1,2,1), plt.imshow(img), plt.title('Original Image'), plt.xticks([]), plt.yticks([])
plt.subplot(1,2,2), plt.imshow(open), plt.title('Skin Image'), plt.xticks([]), plt.yticks([])
plt.show()
